...
const Chat = () => {
    ...
    const fetchThemeFromDB = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:5000/api/chat/theme?sender=${userId}&receiver=${receiver}`);
    ...
    const saveThemeToDB = async (theme) => {
        try {
            ...
            await axios.post(`http://localhost:5000/api/chat/theme`, { sender: userId, receiver: receiver, theme });
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const fetchChatHistory = async (receiverId) => {
        try {
            ...
            const res = await axios.get(`http://localhost:5000/api/chat/${senderId}/${receiverId}`, config);
    ...
    const handleSendMessage = async () => {
        try {
            ...
            const res = await axios.post('http://localhost:5000/api/chat/send', data, config);
    ...
    const handleImageUpload = async () => {
        ...
        try {
            ...
            const res = await axios.post('http://localhost:5000/api/chat/image', formData, {
                headers: {
                    ...config.headers,
    ...
    const handleDocUpload = async () => {
        ...
        try {
            ...
            const res = await axios.post('http://localhost:5000/api/chat/docs', formData, {
                headers: {
                    ...config.headers,
                    'Content-Type': 'multipart/form-data',
                },
                                    ...
                                    <div key={chat._id} className={`chat-message-wrapper ${chat.sender === getUserIdFromToken() ? 'self-end' : 'self-start'}`}>
                                        ...
                                        >
                                            {chat.message ? (
                                                ...
                                                <audio controls>
                                                    <source src={`http://localhost:5000${chat.voiceMessageUrl}`} type="audio/webm" />
                                                    Your browser does not support the audio element.
                                                </audio>
                                            ) : chat.imageUrl ? (
                                                <img src={`http://localhost:5000${chat.imageUrl}`} alt="pic" className="max-w-full h-auto" />
                                            ) : chat.docUrl ? (
                                                <div>
                                                    <DescriptionIcon style={{ verticalAlign: 'iddle', marginRight: '4px' }} />
                                                    <a href={`http://localhost:5000${chat.docUrl}`} target="_blank" rel="noopener noreferrer" style={{ color: mode === 'dark' ? '#fff' : theme.palette.primary.main }}>
                                                        {chat.docUrl.substring(chat.docUrl.lastIndexOf('/') + 1)}
                                                    </a>
                                                </div>
                                            ) : null}
